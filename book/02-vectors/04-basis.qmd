...
::: {.widget-frame}
::: {.widget-predict}
Press play and the grid will glide from the lab's square axes to a crystal's skewed cell axes. Predict which of these changes and which does not: the red arrow's direction; the red arrow's length; the numbers describing it.
:::
::: {.widget-body}
<div style="display:flex; gap:1.2rem; flex-wrap:wrap; align-items:center; margin-bottom:0.6rem;">
  <button id="bcPlay" type="button">▶ lab ⇄ cell axes</button>
  <label>scrub <input type="range" id="bcScrub" min="0" max="100" value="0"></label>
</div>
<canvas id="bcCanvas" width="660" height="420"></canvas>
<div class="widget-readout" id="bcReadout"></div>
:::
::: {.widget-debrief}
The arrow never moves — watch it against the ghost grid — yet its coordinate readout changes continuously as the basis slides beneath it. That is the section's central distinction performed: the vector is physical, the coordinates are a negotiation between the vector and whichever grid is currently asking. Fractional coordinates in a CIF are the far end of this animation, lived in daily.
:::
:::
...
<script src="../js/widgets/basis-change.js"></script>

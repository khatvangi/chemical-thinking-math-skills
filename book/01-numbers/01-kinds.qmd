...
One level of the tower remains. The equation $x^2 = -1$ has no real solution — squares of reals are never negative — and repairing *that* failure produces the complex numbers $\mathbb{C}$, built from $i = \sqrt{-1}$. Chemistry needs them for one great purpose: waves. The electron's wavefunction, the phases that make bonding and antibonding orbitals differ, and the oscillating solutions of Chapter 5's differential equations are all natively complex. We flag the tower's top level now and will climb it when the phenomena demand it — which is precisely how, by Theorem 1.1.2's example, every level of the tower was built in the first place.
